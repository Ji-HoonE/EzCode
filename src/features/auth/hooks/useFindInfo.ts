...
import { useFindPasswordMutation } from "@/query/auth/auth";
...
export const useFindInfo = () => {
    ...
    const handleFindPasswordClick = async () => {
        try {
            const res = await findPasswordMutation({
                email: findEmail,
                redirectUrl: 'http://localhost:3000/find',
            });
    ...
    return { findEmail, handleChangeFindPassword, handleFindPasswordClick, errorMessage, successMessage };
}